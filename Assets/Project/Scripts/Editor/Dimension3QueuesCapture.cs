...
public static class Dimension3QueuesCapture
{
    private const string ActiveKey = "QF.D3QueuesCapture.Active";
    private const string FrameKey = "QF.D3QueuesCapture.Frame";
    private const string FailureKey = "QF.D3QueuesCapture.Failed";
    private const string ScenePath = "Assets/Project/Scenes/Main.unity";
    private static string OutputDirectory =>
        Path.GetFullPath("Logs/VisualQA/Dimension3/Queues");
    private static string Output1080 => Path.Combine(
        OutputDirectory, "Queues_candidate_1080x1920.png");
    private static string Output720 => Path.Combine(
        OutputDirectory, "Queues_candidate_720x1280.png");
    ...
    [MenuItem("Tools/Quantum Forge/Dimension 3/Capture Queues")]
    ...
    private static void OnPlayModeChanged(PlayModeStateChange state)
    {
        ...
        else if (state == PlayModeStateChange.EnteredEditMode)
        {
            ...
            Debug.Log(failed
                ? "[D3 Queues Capture] FAIL"
                : "[D3 Queues Capture] PASS | Producción + chassis V1 x1 activo + 6 s restantes + pagado 100 LE/1 T + confirmación de cancelación + 1080x1920 + 720x1280 | " + Output1080);
    ...
    private static void PrepareRealState()
    {
        ...
        state.dimension3.presentation.lastScreenId = PresentationFeatureIds.D3QueuesFull;
        state.dimension3.assignments.Clear();
        ClearQueues(state.dimension3);

        if (!Dimension3System.TryQueuePartProduction(
                state, Dimension3Catalog.PartChassis, 1, 1L, out string reason))
            throw new InvalidOperationException("No se pudo crear el trabajo canónico: " + reason);
        D3QueueState queue = D3JobQueueSystem.GetQueue(
            state.dimension3, Dimension3Catalog.QueuePartProduction);
        if (queue == null || queue.jobs.Count != 1)
            throw new InvalidOperationException("La cola de producción no contiene el trabajo.");
        D3JobState job = queue.jobs[0];
        job.baseDurationSeconds = 10d;
        job.remainingSeconds = 6d;
        job.paidLE = 100d;
        job.paidTraces = 1d;
        job.started = true;
        job.usesDynamicBankSpeed = true;
        ...
        D3QueuesPanelUI panel = UnityEngine.Object.FindFirstObjectByType<D3QueuesPanelUI>(
            FindObjectsInactive.Include);
        if (dimension == null || panel == null)
            throw new InvalidOperationException("Gestión de colas está incompleta.");
        ...
        panel.Open();
        SetDropdownByText(panel.queueDropdown, "Producción");
        panel.Refresh();
        SetDropdownByText(panel.jobDropdown, "ACTIVO 1 — chassis V1 ×1");
        panel.cancelSelectedButton.onClick.Invoke();
        HideUnrelated();
    ...
    private static void ValidateVisibleState()
    {
        ...
        if (state == null || panel == null || !panel.gameObject.activeInHierarchy)
            throw new InvalidOperationException("Gestión de colas no está visible.");
        D3QueuesStaticSkinUI skin =
            panel.GetComponentInChildren<D3QueuesStaticSkinUI>(true);
        ...
        D3QueueState queue = D3JobQueueSystem.GetQueue(
            state.dimension3, Dimension3Catalog.QueuePartProduction);
        if (queue == null || queue.jobs.Count != 1)
            throw new InvalidOperationException("La cola real no conserva un único trabajo.");
        D3JobState job = queue.jobs[0];
        if (!job.started || job.jobType != Dimension3Catalog.JobPartProduction ||
            job.targetId != Dimension3Catalog.PartChassis || job.version != 1 ||
            job.quantity != 1L || Math.Abs(job.baseDurationSeconds - 10d) > .001 ||
            Math.Abs(job.remainingSeconds - 6d) > .001 ||
            Math.Abs(job.paidLE - 100d) > .001 ||
            Math.Abs(job.paidTraces - 1d) > .001)
            throw new InvalidOperationException(
                "El trabajo real no coincide con la referencia: " +
                "started=" + job.started +
                ", type=" + job.jobType +
                ", target=" + job.targetId +
                ", version=" + job.version +
                ", quantity=" + job.quantity +
                ", base=" + job.baseDurationSeconds +
                ", remaining=" + job.remainingSeconds +
                ", paidLE=" + job.paidLE +
                ", paidT=" + job.paidTraces + ".");
        string detail = panel.detailText == null ? "" : panel.detailText.text;
        string notice = panel.noticeText == null ? "" : panel.noticeText.text;
        if (!detail.Contains("chassis V1 ×1") || !detail.Contains("Estado: Activo") ||
            !detail.Contains("Trabajo restante: 6") ||
            !detail.Contains("Tiempo estimado actual: 6 s") ||
            !detail.Contains("Pagado: 100 LE + 1 T") ||
            !notice.Contains("Trabajo activo") || !notice.Contains("5 s"))
            throw new InvalidOperationException(
                "El detalle/aviso real no coincide: " + detail + " | " + notice);
        if (panel.cancelSelectedButton == null ||
            !panel.cancelSelectedButton.interactable || panel.backButton == null ||
            !panel.backButton.interactable || panel.queueDropdown == null ||
            !panel.queueDropdown.interactable || panel.jobDropdown == null ||
            !panel.jobDropdown.interactable)
            throw new InvalidOperationException("Los controles reales no están disponibles.");
