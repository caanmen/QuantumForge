...
public static class Dimension2ResistanceAlertCapture
{
    private const string ActiveKey = "QF.D2ResistanceAlertCapture.Active";
    private const string FrameKey = "QF.D2ResistanceAlertCapture.Frame";
    private const string FailureKey = "QF.D2ResistanceAlertCapture.Failed";
    private const string SaveFingerprintKey = "QF.D2ResistanceAlertCapture.Save";
    private const string BackupFingerprintKey = "QF.D2ResistanceAlertCapture.Backup";
    ...
    private static string Output1080 => Path.Combine(OutputDirectory,
        "D2_Alerta_Del_Ente_1080x1920.png");
    private static string Output720 => Path.Combine(OutputDirectory,
        "D2_Alerta_Del_Ente_720x1280.png");
    ...
    private static void OnPlayModeChanged(PlayModeStateChange state)
    {
        ...
        else if (state == PlayModeStateChange.EnteredEditMode)
        {
            ...
            EditorApplication.playModeStateChanged -= OnPlayModeChanged;
            Debug.Log((failed ? "[D2 Resistance Alert Capture] FAIL | " :
                "[D2 Resistance Alert Capture] PASS | STRUCTURE_PASS | STATE_PASS | " +
    ...
    private static void PrepareFixtureAndMap()
    {
        ...
        gameState.dimension2.civilization2 = new D2Civilization2State
        {
            ...
            alertMarkProgressSeconds = 222.0,
            totalAlertMarks = 2L,
            lastAlertResult =
                "Región 1 fue marcada: la próxima Represalia añade 3% de pérdidas."
        };
        D2Civilization2State resistance = gameState.dimension2.civilization2;
        D2Civilization2System.EnsureState(resistance);
        ConfigureRegion(resistance, D2Civilization2System.Region1Id, true, true);
        ConfigureRegion(resistance, D2Civilization2System.Region2Id, false, false);
        ConfigureRegion(resistance, D2Civilization2System.Region3Id, false, false);
    ...
    private static void ConfigureRegion(
        D2Civilization2State state, string id, bool marked, bool protectedRegion)
    {
        D2RegionState region = D2Civilization2System.GetRegion(state, id);
        Require(region != null, "Falta " + id + ".");
        region.unlocked = true;
        region.dominance = 30.0;
        region.threat = id == D2Civilization2System.Region1Id ? 72.0 : 40.0;
        region.membersAssigned = 100L;
        region.alertMarked = marked;
        D2OperationState protection = D2Civilization2System.GetOperation(
            region, D2Civilization2System.ProtectionOperationId);
        Require(protection != null, "Falta Protección en " + id + ".");
        protection.membersAssigned = protectedRegion ? 5L : 0L;
    }
    ...
    private static void RestoreFixtureVisualState()
    {
        ...
        state.alertMarkProgressSeconds = 222.0;
        state.totalAlertMarks = 2L;
        state.lastAlertResult =
            "Región 1 fue marcada: la próxima Represalia añade 3% de pérdidas.";
        ConfigureRegion(state, D2Civilization2System.Region1Id, true, true);
        ConfigureRegion(state, D2Civilization2System.Region2Id, false, false);
        ConfigureRegion(state, D2Civilization2System.Region3Id, false, false);
        Dimension2PanelUI panel = UnityEngine.Object.FindFirstObjectByType<Dimension2PanelUI>(
            FindObjectsInactive.Include);
        Require(panel?.civilization2PanelUI?.alertPanelUI != null,
            "No existe la UI para restaurar el fixture.");
        panel.civilization2PanelUI.alertPanelUI.Refresh();
        Canvas.ForceUpdateCanvases();
    }
    ...
    private static void ValidatePhysicalRoutesAndRestore()
    {
        ...
        Require(panel != null && civilization?.alertPanelUI != null,
            "Falta la pantalla de Alerta.");
        ...
        Require(civilization.resistanceSectionRoot.activeInHierarchy,
            "RESISTENCIA no abrió desde Alerta.");
        ClickPhysical(civilization.showAlertButton);
        Require(civilization.alertSectionRoot.activeInHierarchy,
            "ALERTA no se restauró.");
    ...
    private static void ValidateVisibleState()
    {
        Dimension2PanelUI panel = UnityEngine.Object.FindFirstObjectByType<Dimension2PanelUI>(
            FindObjectsInactive.Include);
        D2Civilization2PanelUI civilization = panel?.civilization2PanelUI;
        D2AlertPanelUI ui = civilization?.alertPanelUI;
        Require(ui != null && civilization.alertSectionRoot.activeInHierarchy &&
            ...
            "Alerta no es la única sección visible.");
        Require(ui.stateText.text == "EL ENTE HA ENTRADO EN ALERTA" &&
            ui.dominanceText.text == "30%" && ui.timerText.text == "06:18" &&
            ui.marksCountText.text == "2" &&
            ui.regionMarkTexts[0].text.StartsWith("MARCADA") &&
            ui.regionMarkTexts[1].text == "SIN MARCA" &&
            ui.regionProtectionTexts[0].text == "PROTECCIÓN\nACTIVA" &&
            ui.lastResultText.text == "REGIÓN 1 FUE MARCADA" &&
            ui.lastResultDetailText.text ==
                "LA PRÓXIMA REPRESALIA AÑADE 3% DE PÉRDIDAS",
            "Los datos V4 de Alerta no coinciden.");
        Require(Mathf.Abs(ui.dominanceGauge.Progress - 0.30f) < 0.001f,
            "El medidor no representa el Dominio real.");
        ...
        Require(Mathf.Abs(root.rect.width - 1080f) <= 1f &&
            ...
            "Alerta no conserva el lienzo 1080x1920.");
        ValidateTextGeometry(root);
        Debug.Log("[D2 Resistance Alert Capture] STRUCTURE_PASS | STATE_PASS | " +
